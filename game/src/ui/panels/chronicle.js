...
export function renderChronicle(ctx, node) {
  ...
  node.appendChild(el('div', { class: 'lead-note' },
    world.intel >= 2 ? '本当の原因まで見えている。'
      : world.intel >= 1 ? '疑わしい説明には印が付いている。'
      : '調べる手立てが無い。ここに並ぶのは長が言ったことだけ。嘘も混ざっている。'));
  ...
  for (const e of list) {
    ...
    mount(card, 
      el('div', { style: { display: 'flex', gap: '7px', alignItems: 'baseline' } },
        ...
        el('span', { style: { flex: 1, fontSize: '14px' } }, e.text || ''),
      ),
...
function expand(ctx, e) {
  ...
  // ---- 上流
  box.appendChild(el('div', { style: { fontSize: '13.5px', fontWeight: '700', marginBottom: '5px' } }, '何が原因か'));
  if (e.trueCause == null) {
    ...
    box.appendChild(el('div', { class: 'card', style: { background: '#171207', borderColor: '#4a3c1c', margin: '0 0 6px' } },
      el('div', { class: 'tagrow', style: { marginTop: 0, marginBottom: '4px' } },
        el('span', { class: 'tag warn' }, '長が言っていること'),
        world.intel >= 1 ? el('span', { class: 'tag bad' }, '疑わしい') : null),
      el('p', {}, e.claimed || '（説明がない）'),
      el('p', { class: 'hint' }, 'これ以上は遡れない。数字は正確でも、なぜそうなったかは数字の中に無い。'),
    ));
  }

  // ---- 下流
  box.appendChild(el('div', { style: { fontSize: '13.5px', fontWeight: '700', margin: '12px 0 5px' } }, '何を引き起こしたか'));
  ...
  if (e.effects && e.effects.length) {
    box.appendChild(el('div', { style: { fontSize: '13.5px', fontWeight: '700', margin: '12px 0 5px' } }, '効いた先'));
  ...
  if (api.canonize && !e.canon && (e.claimed || e.trueCause != null)) {
    box.appendChild(el('div', { style: { fontSize: '13.5px', fontWeight: '700', margin: '14px 0 5px' } }, 'この件の「正しい話」を決める'));
    box.appendChild(el('p', { class: 'hint' }, '一度決めると、以後この件はその話で通る。'));
...
function refRow(ctx, e, arrow) {
  return el('div', {
    ...
    el('div', { class: 'v', style: { fontWeight: 400, fontSize: '13.5px', textAlign: 'right', flex: 1 } }, e.text || ''),
  );
}
