...
check('**12本とも決定的で、使っているストリームを動かせば別世界になる**（#17 §10-3）', () => {
  const trace = (burn) => {
    ...
    //   予備（11）… 2026-08-31 に負傷の治癒、2026-08-31 に斥候（#17 §6-8）で使い始めた
    //   ★★ 2026-09-01：**12本とも実際に使われている ＝ 空撃ちできるストリームが1本も無い。**★★
    //     それまで予備(11)で空撃ちしても緑だったのは、**引いた値が世界に届いていなかった**から
    //     （`healMonth` が `w >= 3` でしか使わず、`hurtStage` に 3 を書く行が1つも無かった。
    //      正典4724 の第2条件「手当ての担い手がいない村で治癒」を入れて初めて生きた）。
    //     ＝ **この検査は「未使用の1本」ではなく「使い始めた1本が他を汚さないこと」で見る。**
    //     いま見るのは **12本すべての `saveRandom` の往復**（別の検査が持っている）と、
    //     **同じ種を2回走らせて1ビットも違わないこと**。ここでは後者を見る
    void burn;   // ★ もう空撃ちしない（空撃ちできる本が無い）
    const t = [];
    for (let y = 0; y < 80; y++) {
      w.runYears(1);
      t.push(`${w.tick}:${w.people.aliveCount()}:${w.houses.count}:${w.villages.a.food[0].toFixed(4)}`);
  ...
  // ★ 同じ種を3回走らせて1ビットも違わないこと（決定性の芯）
  const none = trace(0);
  for (const again of [0, 0, 0]) {
    if (trace(again) !== none) return '同じ種で2回走らせたら歴史が変わった（決定的でない）';
  }
...
check('**人口が爆発も絶滅もしない**（100年・20通り）', () => {
  ...
  // ★ 2026-09-01：**250 は「育つと赤くなる罠」だった**（実測232 で残り7%・第2回の精査の指摘）。
  //   見たいのは「**指数で爆発していない**」ことなので、100年で人口が10倍を超えたら赤にする
  //   （創世は十匹なので、100年で 250 は年率3.3% ── 前近代の上限より速い。1,000 は年率4.7%）
  if (max > 1000) return `いちばん増えた世界で ${max} 人（爆発）`;
  ...
  //   ★★ 2026-08-31：**下側（＝1つは絶滅すること）の判定をここから外した。**★★
  //     真の絶滅率が 10%（120年・40通り）なら、100年・n=20 で**偶然0になる確率が
  //     36%**あり、機構が無傷でも3回に1回赤くなる。**下側は次の検査
  //     「絶滅率が実測（創世十匹で10%）と近い（120年・40通り）」が帯で見ている。**
  //     ここに残すのは上側（爆発しない）と中央値だけにする。
  if (ext > 11) return `20通り中 ${ext} が絶滅（多すぎる）`;
