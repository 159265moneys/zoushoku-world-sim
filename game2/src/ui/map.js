...
export const AREA_RECT = [
  ...
  // ★ 2026-08-31：**5個のままだったので工事人と漁師が住居の箱の中に描かれていた**
  //   （`AREA_RECT[5]`／`[6]` が undefined → 既定で 0 番に落ちる）。
  //   どちらも専用の絵を持たないので、**いちばん近い意味の帯に寄せる**：
  //   工事は畑と同じ野良仕事なので畑の帯の下、漁は森（獲る）の帯の下に細く置く
  { x: 60,  y: 718, w: 920, h: 38  },   // 5 工事（畑の帯 548+168=716 の下・VH760 に収める）
  { x: 60,  y: 288, w: 496, h: 16  },   // 6 漁（森の帯 46+240=286 と住居 306 のあいだ）
];
...
const COL = {
  ...
  labelHi:      '#cdc6ae',
  sel:          '#f2ead2',
  // 弔いの×。**赤をやめた。**A-5「色に意味を載せない」に、盤面で唯一残っていた違反。
