...
export const SPEC = {
  ...
  kills: 'u16',          // 討ち取った数（通算）
  battles: 'u8',         // 戦に出た回数（T7・通算）
  // ★ 直近12ヶ月に交戦した月のビット輪（#5 §5025「憤怒 U … ← **直近12ヶ月**」）。
  //   2026-09-01（第2回の精査で発見）：`unmetWrath` に**通算の `battles`** を渡していたので、
  //   生涯4戦で憤怒が**永久に満充足**になっていた。毎月1つ左へ送り、戦に出た月に bit0 を立てる。
  //   立っているビットの数がそのまま「直近12ヶ月の交戦月数」
  warRing: 'u16',
