...
// ★ 斥候（#17 §6-8）。★★ 2026-09-01（第2回の精査で発見）：**代金を1/4しか払っていなかった。**
//   `AREA_HOME` を1回書くだけだったので翌月 `assignWork` が畑へ配り直し、
//   **斥候人月の68〜74%が `AREA_FIELD`** に戻っていた（正典9540「産出に出ない」が不成立）。
//   専用の職域を持たせて、配役から外す
export const AREA_SCOUT = 7;
// ★ 2026-08-31：**5個のままだったので画面に `undefined` が出ていた**（別セッションの精査で発見）。
//   `AREA_BUILD=5`（工事）と `AREA_FISH=6`（漁）を足したのに名前の表を伸ばしておらず、
//   村の札が人口と合わなかった（`byArea[6]` が NaN）。**職域を足したら必ずここも足す。**
export const AREA_COUNT = 8;
export const AREA_NAMES = ['住居', '畑', '森', '訓練場', '辺境', '工事', '漁', '斥候'];
...
export function assignWork(P, V, tick, land = null) {
  ...
  for (let i = 0; i < A.len; i++) {
    ...
    if (A.job[i] !== AREA_HOME) continue;             // もう振ってある（斥候・工事もここで外れる）
    // ★★ 漁は**森を押しのける。畑は押しのけない。**★★
