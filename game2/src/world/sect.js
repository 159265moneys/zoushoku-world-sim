...
// ★★ 2026-08-31：**55 → 45 に校正した。**★★
//   正典6485 は「誘い手 ＝ 同じ村の s の信徒（faith ≥ 35）のうち、i への好き嫌い[→i] が
//   **55 以上**」と書き、同じ正典の検算(a)（6633）は「宗派A 60%（＝信徒36人）のとき
//   **誘い手18人**」＝ **信徒のちょうど半分が誘い手**を前提に p=0.00151/月 を出している。
//   ★ この2つは同じ分布から出ていない。実測（種3・300年・同村の大人 25,592組）：
//       好き嫌いの平均 **43.9**／**≥55 は 19.7%**／**≥45 は 51.7%**
//     ＝ 55 では検算の前提（50%）の**4割**しか誘い手にならず、伝播が半分以下になる。
//   → **検算が成り立つ側（45）を採る。**理由は 746→450 や当たり率の3点と同じで、
//     正典が「規則」と「その帰結」の両方を書いていて食い違うとき、
//     **他の数（0.00151／0.00038／「Aが4倍有利」／分布が収束する議論）が全部乗っている
//     帰結のほう**を守る。式そのものは1文字も変えていない（実測 0.00140 で検算と一致）。
export const INVITE_LIKE = 45;       // 誘い手：好き嫌い[→i] がこの値以上
...
import { T_FAITH as TIE_FAITH, affinityTable, feelWith } from './ties.js';
...
export function beliefMonth(P, V, sects, ties, tick, rng, chiefSect = SECT_NONE) {
  ...
  // ★ 鍵は**数値**にする。`${v},${s}` の文字列連結は人数ぶん走るので、
  //   1,304人の実測でここが beliefMonth の重さの一因だった。宗派IDは 4096 未満に収まる
  const KEY = (v, sx) => v * 4096 + sx;
  const bySect = new Map();                        // KEY(v,s) → 人数
  ...
  const roster = new Map();                        // KEY(v,s) → 添字の配列
  for (let i = 0; i < A.len; i++) {
    if (!A.alive[i] || (A.ageMonths[i] / 12 | 0) < FOUND_AGE) continue;
    const v = A.village[i];
    if (v === NO_VILLAGE || v >= nv) continue;
  ...
  for (let i = 0; i < A.len; i++) {
    ...
    // ---- §4 faith の月次。★飽和させる ----
    if (s && SA.alive[s]) {
      const same = (bySect.get(KEY(v, s)) ?? 0) / Math.max(1, pop12[v]);
  ...
  // ---- §6 伝播（入信・改宗）----
  //   ★ 誘い手を数えるのに好き嫌いを何度も引くので、相性の表を1枚だけ作ってから読む
  ...
  for (let i = 0; i < A.len; i++) {
    ...
    for (let s = 1; s < SA.len; s++) {
      ...
      if (!(bySect.get(KEY(v, s)) > 0)) continue;
      // 誘い手：同じ村の s の信徒（faith ≥ 35）のうち、i への好き嫌い[→i] が 55 以上 の人数
      // ★ 名簿から引く。全人口を走らない
      let invite = 0;
      for (const j of roster.get(KEY(v, s)) ?? EMPTY) {
        if (feelWith(tab, ties, j, i) >= INVITE_LIKE) invite++;
      }
    ...
    if (cur) {
      ...
      for (const j of roster.get(KEY(v, cur)) ?? EMPTY) ties.link(j, i, CONVERT_HATE, TIE_FAITH, mon);
