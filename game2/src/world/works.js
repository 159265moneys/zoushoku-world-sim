...
// §4-3 工事キュー　★村ごと同時1件
// ---------------------------------------------------------------------------
export const MIN_FOREST = 3;   // §2-2 の標準村が要る森林の枚数（創世の村の畑を開くときの下限）
...
export function pickParcel(land, L, v, toRole) {
  ...
  for (const p of cells) {
    ...
    // ★★ **拠点地は絶対に潰さない**（2026-08-31・別セッションの精査で発見）★★
    //   人工どうしの変換は30人月で、平野→畑の60人月より**安い**。だから
    //   `いちばん安い区画` を選ぶこの関数は **1件目に必ず村の拠点地を選んでいた**。
    //   しかも拠点地は状態値を持たない（land.js が `R.HOME` を素で書く）ので
    //   **地力0の畑**ができ、recap が平均するので村の産出が恒久的に落ちていた
    //   （種3で産出倍率 1.000→0.907／種6は1村が0.000で死亡／100年後に世界から拠点地が消える）。
    //   正典8810「分村が作る最初の拠点地区画だけは 11-B が直接書き込む。**これが無いと
    //   新村は自分の拠点地を建てられず、分村そのものが成立しない**」＝ 潰してよいものではない。
    if (from === R.HOME) continue;
    // ★ 森林を「3枚残す」禁止は**入れて外した**（2026-08-31）。
    //   精査の指摘「開墾が森を食い尽くし狩人が全員0産出になる」は正しかったが、
    //   塞ぐ場所が違った ── **開墾を止めると畑が足りなくなって世界が死ぬ**
    //   （実測：120年40種の絶滅率 8% → **38%**）。
    //   正しい塞ぎは `village.js` の `assignWork` 側で、**森の定員を超えたぶんを畑へ回す**
    //   （crowd は硬い天井なので、超えて入れた人は0産出になるだけ）。それだけで 5%／60.4。
